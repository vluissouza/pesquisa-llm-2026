...
\tikzstyle{iniciofim} = [rectangle, rounded corners, minimum width=3cm, minimum height=1cm, text centered, draw=black]
\tikzstyle{processo} = [rectangle, minimum width=3cm, minimum height=1cm, text centered, draw=black]
\tikzstyle{decisao} = [diamond, aspect=2, text centered, draw=black]
...
\noindent\textbf{Keywords:} EV charging infrastructure; interoperability; technological standardization; CCS2; electric mobility; infrastructure efficiency.
...
\node (dados) [processo, below of=inicio] {Coleta de Dados};
\node (anova) [processo, below of=dados,] {ANOVA};
\node (decisao) [decisao, below of=anova, yshift=-1cm] {p-value < 0.05?};
\node (tukey) [processo, below of=decisao, yshift=-1cm] {Teste de Tukey};
\node (reg) [processo, right of=decisao, xshift=4cm] {Regressão};
\node (fim) [iniciofim, below of=tukey] {Conclusão};

\draw [seta] (inicio) -- (dados);
\draw [seta] (dados) -- (anova);
\draw [seta] (anova) -- (decisao);
\draw [seta] (decisao) -- node[left]{Sim} (tukey);
\draw [seta] (decisao) -- node[above]{Não} (reg);
\draw [seta] (tukey) -- (fim);
...
\begin{figure}[h]
\centering
\includegraphics[width=0.4\linewidth]{figures/ccs-tipo-1.png}
\caption{Conector CCS Tipo 1}
\label{fig:ccs-tipo-1}
\end{figure}
